...
describe("ProspectForm", () => {
  it("shows inline validation messages for invalid input", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /generate stub preview/i }));
  ...
  it("renders the stub success shell for valid input", async () => {
    render(<ProspectForm />);
    ...
    expect(await screen.findByText(/shaping the stubbed handoff/i)).toBeTruthy();
    expect(
      await screen.findByText(/stub preview ready/i, undefined, { timeout: 2000 }),
    ).toBeTruthy();
    expect(
  ...
  it("renders the fallback error shell when the error path is requested", async () => {
    ...
    expect(await screen.findByText(/shaping the stubbed handoff/i)).toBeTruthy();
    expect(
      await screen.findByText(/the stub hit a deliberate failure path/i, undefined, {
        timeout: 2000,
      }),
    ).toBeTruthy();
    expect(
      await screen.findByText(/verify the fallback copy before the real orchestration route exists/i),
    ).toBeTruthy();
  });
});
